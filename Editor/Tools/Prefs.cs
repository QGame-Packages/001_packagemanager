    ...
    public struct Prefs<T>
    {
        ...
        public T Value
        {
            get
            {
                ...
                if (this.m_accessor.HasKey(this.m_key))
                {
                    this.m_value = this.m_accessor.Get(this.m_key);
                }
                else if (this.m_groupKey != null && this.m_accessor.HasKey(this.m_valueKey))
                {
                    //第一次时，如果定向的值拿不到，就去尝试拿全局的值
                    this.m_value = this.m_accessor.Get(this.m_valueKey);
                }
                else
                {
                ...
                this.m_rwFlag.MarkSelf(RwFlag.Read);
