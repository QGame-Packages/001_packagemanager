...
namespace ET.Editor.PackageManager
{
    ...
    public partial class ETPackageVersionModule : BasePackageToolModule
    {
        ...
        private EnumPrefs<EPackagesFilterType> FilterTypePrefs =
            new("ETPackageVersionModule_FilterType", null, EPackagesFilterType.ET);

        private EnumPrefs<EPackagesFilterOperationType> FilterOperationTypePrefs =
            new("ETPackageVersionModule_FilterOperationType");
...
#pragma warning restore CS4014 // 由于此调用不会等待，因此在调用完成前将继续执行当前方法
        ...
        private async Task UpdatePackagesInfo(bool close = true)
        {
            ...
            foreach (var data in m_AllPackageInfoDataDic)
            {
                ...
                if (!CheckPackageChange(packageInfo))
                {
                    continue;
                }
        ...
        private bool CheckPackageChange(PackageVersionData packageInfoData)
        {
            ...
            for (var i = 0; i < packageInfoData.Dependencies.Count; i++)
            {
            ...
            for (var i = 0; i < packageInfoData.DependenciesSelf.Count; i++)
            {
        ...
        private async Task ChangePackageInfo(PackageVersionData packageInfoData)
        {
            ...
            try
            {
                ...
                await File.WriteAllTextAsync(packagePath, json.ToString(), Encoding.UTF8);
        ...
        {
            m_AllPackageInfoDataDic.TryGetValue(packageName, out var packageInfoData);
        ...
        private void LoadAllPackageInfoData()
        {
            m_AllPackageInfoDataDic =
                (Dictionary<string, PackageVersionData>)SerializationUtility.CreateCopy(
                    PackageVersionHelper.PackageVersionAsset.AllPackageVersionData);
        ...
        {
            if (m_VersionValueDict.TryGetValue(version, out var value))
            {
                return value;
            }
